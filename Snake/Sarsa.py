...
class Learner(object):
    def __init__(self, display_width, display_height, block_size):
        # Game parameters (Use for surroundings)
    ...
    # This function is called if the snake dead
    def Reset(self):
        self.history = []
    ...
    def UpdateQValues(self, reason):
        ...
        if reason: # Snake Died -> Negative reward
            ...
            s1 = history[0]['state'] # current state
            s0 = history[1]['state'] # previous state
            a0 = history[1]['action'] # action taken at previous state
            ...
            y2 = s1.distance[1] # y distance at current state
    ...
    def _GetState(self, snake, food): # -> GameState(distance, position, surroundings, food)
        snake_head = snake[-1]
